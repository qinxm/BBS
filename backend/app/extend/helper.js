...
exports.getCookieObject = ctx => {
  let obj = {},
    cookieStr = ctx.request.header.cookie;
  let reg = /([^?&=;]+)=([^?&=;]*)/g;
  cookieStr &&
    cookieStr.replace(reg, function(rs, $1, $2) {
...
exports.verifyToken = async (ctx, userObj) => {
  ...
  if (
    userObj.userId != verifyResult.message.id &&
    userObj.authorid != verifyResult.message.id
  ) {
